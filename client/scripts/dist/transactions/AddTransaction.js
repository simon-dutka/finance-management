const addTransaction = () => {
    let formTransactions = document.querySelectorAll('.form-transactions');
    ...
    formBtns.forEach((element, i) => {
        ...
        element.addEventListener('click', () => {
            const saveTransaction = () => {
                ...
                for (let j = 0; j < 4; j++) {
                    transactionValues.push(`elVal-${formTransactions[i].children[j].value} `);
                }
            ...
            createdTransactions === null
                ...
                : localStorage.setItem('allTransactions', saveTransaction() + createdTransactions);
        });
    });
};
