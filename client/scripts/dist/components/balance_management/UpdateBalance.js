const updateBalanceIncome = () => {
    ...
    for (let i = 0; i < historyContainer.children.length; i++) {
        if (historyContainer.children[i].classList.contains('transaction__container--income')) {
    ...
    for (let i = 0; i < historyContainer.children.length; i++) {
        if (historyContainer.children[i].classList.contains('transaction__container--expense')) {
    ...
    const saveBalance = () => {
        balanceElement.innerHTML = `Total balance ${balanceValue + Number(localStorage.getItem('startBalance'))} $`;
